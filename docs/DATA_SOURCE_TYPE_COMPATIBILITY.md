...
`oracle`, `sqlserver`, `duckdb`, `clickhouse`, and `apache_doris` are explicitly unsupported and must return structured
...
return `SkippedOp` for ALTER variants; `oracle`, `sqlserver`, `duckdb`, `clickhouse`, and `apache_doris` are explicitly
...
|---|---|
| ER Tabs (Inspector + Designer) | Inspector: mysql / postgresql / h2 / mariadb fully via JDBC `getImportedKeys`, and sqlite user file connections use the same metadata path with no extra schema selector; oracle / sqlserver / duckdb / clickhouse / apache_doris `dialect_unsupported`. Designer day-1 DDL generation: mysql / postgresql / h2 / mariadb emit CREATE TABLE / ALTER ADD COLUMN / ALTER ADD FK / CREATE INDEX; sqlite is CREATE-only with all ALTER variants returning `SkippedOp`; oracle / sqlserver / duckdb / clickhouse / apache_doris `dialect_unsupported`. DROP / ALTER COLUMN type / RENAME are always `SkippedOp` (`day1_unsupported`) regardless of dialect; users must write that SQL manually in the `query_editor` and run it through L2/L3 confirmation. |
