...
| `starrocks` | First-class OLAP SQL support | Connection UI (host/port/username/password, default FE query port 9030), JDBC URL (`jdbc:starrocks://` via native StarRocks Connector/J, `default_catalog.database` composition), database context selector (required), MySQL SQL splitter reuse, StarRocks-specific risk rules (SHOW/DESCRIBE/EXPLAIN L1, INSERT/CREATE TABLE/CREATE INDEX/ANALYZE L2, DROP/TRUNCATE/ALTER/GRANT/REVOKE/CREATE USER/ROLE/CATALOG/DROP CATALOG/LOAD/ROUTINE LOAD/STREAM LOAD/BROKER LOAD/CANCEL LOAD/EXPORT/ADMIN/SET GLOBAL/SET PASSWORD/KILL/SUBMIT TASK/CANCEL TASK/RENAME/INSERT OVERWRITE L3), structured unsupported diagnostics, structured unsupported ER. Day-2: EXPLAIN diagnostics, ER DDL generation, explicit catalog support, real connection smoke. |
...
`oracle`, `sqlserver`, `duckdb`, `clickhouse`, `apache_doris`, `starrocks`, `trino`, and `presto` are explicitly unsupported and must return structured
...
return `SkippedOp` for ALTER variants; `oracle`, `sqlserver`, `duckdb`, `clickhouse`, `apache_doris`, `starrocks`, `trino`, and `presto` are explicitly
