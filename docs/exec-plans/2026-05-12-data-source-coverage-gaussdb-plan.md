...
- [x] **Step 1:** Add GaussDB driver dependency to `server/data-talk-infrastructure/pom.xml`
...
- [x] **Step 2:** Add `GAUSSDB` constant + normalize + reject to `ConnectionKind.java`
...
- [x] **Step 3:** Add GAUSSDB branch in `JdbcUrlBuilder.java`
...
- [x] **Step 4:** Add `"gaussdb"` to PG splitter routing in `DefaultSqlStatementSplitters.java` line 27

Change: `"kingbase".equalsIgnoreCase(connectionKind)` → also include `"gaussdb".equalsIgnoreCase(connectionKind)`

- [x] **Step 5:** Compile verify
...
- [x] **Step 1:** Add `GAUSSDB_SYSTEM_SCHEMAS` constant to `ConnectionTargetDiscoveryService.java`
...
- [x] **Step 2:** Add GAUSSDB branch in `ConnectionService.testConnection`
...
- [x] **Step 3:** Add GAUSSDB to SqlExecuteService kind gate (if `connectionKind` switch exists)

Check for any switch statement routing by kind and add `gaussdb` alongside `kingbase` / `postgresql`.

- [x] **Step 4:** Compile verify
...
- [x] **Step 1:** Add GaussDB-specific L3 patterns
...
- [x] **Step 2:** Add `classifyGaussdbSpecific` method
...
- [x] **Step 3:** Wire into main classify dispatch — add GAUSSDB branch alongside existing KINGBASE branch

- [x] **Step 4:** Write test — `CalciteSqlRiskAnalyzerTest` add gaussdb test cases

- [x] **Step 5:** Compile + test verify
...
- [x] **Step 1:** Create `GaussDBDiagnosticsProvider.java`
...
- [x] **Step 2:** Add i18n keys for diagnostics unsupported messages
...
- [x] **Step 1:** Create `GaussDBSplitterEquivalenceTest.java`

Follow `KingbaseSplitterEquivalenceTest` pattern: 6 test cases using `PostgresJdbcSqlStatementSplitter`.

- [x] **Step 2:** Create `GaussDBDriverCoexistenceTest.java`
...
- [x] **Step 3:** Add gaussdb routing test to `DefaultSqlStatementSplittersTest.java`
...
- [x] **Step 1:** Create `gaussdb-connection-fields.tsx`
...
- [x] **Step 2:** Add gaussdb routing in `connection-form.tsx`

- [x] **Step 3:** Add gaussdb display name in `connection-picker.tsx`

- [x] **Step 4:** Add gaussdb formatter routing in `format-sql.ts` (route to PG formatter)
...
- [x] **Step 1:** Add `"gaussdb"` to ConnectionObjectType enum list

- [x] **Step 2:** Add `## GaussDB` section to AGENTS.md
...
- [x] **Step 2:** Frontend type check + test

Run: `cd client && npx tsc --noEmit && npx vitest run`
Expected: 0 errors, all pass

- [x] **Step 3:** Update `docs/exec-plans/index.md` — move this plan to Completed

- [x] **Step 4:** Update `docs/DATA_SOURCE_TYPE_COMPATIBILITY.md` — add gaussdb row

- [x] **Step 5:** Update `docs/exec-plans/2026-04-25-next-implementation-roadmap-plan.md` — Task 9 Wave C 6/6 complete
