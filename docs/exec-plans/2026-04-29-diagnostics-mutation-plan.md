...
- [x] 把本计划写入 `docs/exec-plans/2026-04-29-diagnostics-mutation-plan.md`（已写）。
- [x] 在 `docs/exec-plans/index.md` 的「活跃计划」表格新增一行：链接 + 创建日期 `2026-04-29` + 摘要（参考其他 plan 摘要风格）。
- [x] 自检 `docs/product-specs/index.md` 第 256 行的 spec 摘要已对齐当前修订（已对齐 Oracle unsupported 描述）。
- [x] commit：`docs(exec-plans): register diagnostics & mutation actions plan`
...
- [x] 修改 `DiagnosticCapability.java`：把 `CONNECTION_POOL` 重命名为 `POOL_STATUS`，新增 `TERMINATE_SESSION` 和 `OPTIMIZE_TABLE`。最终 7 个值：`EXPLAIN, INDEX_HINTS, LOCK_INFO, POOL_STATUS, TABLE_SPACE, TERMINATE_SESSION, OPTIMIZE_TABLE`。
- [x] 重写 `LockReport.java` 为 `LockReport(List<LockEntry> blockingChain, List<DiagnosticRecommendation> recommendations)`；嵌套 record `LockEntry(String table, String lockType, String holderId, String waiterId, Long waitMillis, String holderSql, String waiterSql)`。
- [x] 重写 `PoolReport.java` 为 `PoolReport(String scope, Integer activeConnections, Integer idleConnections, Integer maxConnections, Integer threadsRunning, Integer waitingConnections, String identifier, List<DiagnosticRecommendation> recommendations)`。
- [x] 修改 `SpaceReport.java` 为 `SpaceReport(List<TableSpaceEntry> tables, List<DiagnosticRecommendation> recommendations)`；嵌套 record 加字段：`TableSpaceEntry(String table, String schemaName, long rowCount, long dataSizeBytes, long indexSizeBytes, Long freeSpaceBytes)`。
- [x] 创建 `DiagnosticRecommendation.java`：`record DiagnosticRecommendation(String severity, String summary, String suggestedActionId, String suggestedToolName, Map<String,Object> suggestedActionArgs, String suggestedSql)`。
- [x] 创建 `TerminateSessionPreview.java`：`record TerminateSessionPreview(String engine, String sessionId, String willRunSql, String currentSql)`。
- [x] 创建 `TerminateSessionResult.java`：`record TerminateSessionResult(boolean ok, String sessionId, String message)`。
- [x] 创建 `OptimizeTablePreview.java`：`record OptimizeTablePreview(String engine, String table, String schemaName, String willRunSql, Long currentDataFree, Long currentTotalSize, List<DiagnosticRecommendation> recommendations)`。
- [x] 创建 `OptimizeTableResult.java`：`record OptimizeTableResult(boolean ok, String table, String schemaName, Long durationMs, Long reclaimedBytes, String message)`。
- [x] 编译验证：`cd server && mvn compile -q -pl data-talk-domain` 必须零错。
- [x] **关键收尾**：`mvn install -pl data-talk-domain -am -DskipTests` 把新 jar 推到 `~/.m2`，否则后续 batch 编译拉到旧版（CLAUDE.md "Backend Run vs Compile" 强制要求）。
- [x] commit：`feat(domain): add diagnostics records & capability enum for mutation actions`
...
- [x] 创建 `DiagnosticsThresholdProperties.java`（`@ConfigurationProperties("datatalk.diagnostics")`）。结构按 spec §5：
...
- [x] 在 `server/data-talk-adapter/src/main/resources/application.yml` 加默认值：
...
- [x] 同样写入 `server/data-talk-adapter/src/test/resources/application.yml`，避免测试 profile 拿不到默认值。
- [x] 在 application 模块的 `@SpringBootApplication`/configuration class 启用 `@EnableConfigurationProperties(DiagnosticsThresholdProperties.class)`（如果当前已有总注册点就加一行；如果没有就在 `DiagnosticsService` 类上加 `@EnableConfigurationProperties(DiagnosticsThresholdProperties.class)`）。

### 3.2 DiagnosticsProvider 接口扩展

- [x] 修改 `DiagnosticsProvider.java`：
...
- [x] **RED**：先写 `DiagnosticsServiceTest`，新增/补充覆盖：
...
- [x] 跑 `mvn test -pl data-talk-application -Dtest=DiagnosticsServiceTest`，确认全 RED。
- [x] **GREEN**：在 `DiagnosticsService` 加：
...
- [x] 跑 `mvn test -pl data-talk-application -Dtest=DiagnosticsServiceTest`，全 GREEN。

### 3.4 DiagnosticsService — mutation 方法 (4 个)

- [x] **RED**：在 `DiagnosticsServiceTest` 加：
...
- [x] `cd server && mvn compile -q` 全模块零错（此时 infra 层 4 个 provider 还没改，会因方法签名不匹配编译失败——需要先打通 application 层编译，infra 在 Task 4-7 处理）。
- [x] **注意**：因为 `DiagnosticsProvider` 接口扩了，4 个 provider 在 Task 4 之前都不能编译。所以本 task 只能跑到 `mvn compile -pl data-talk-application -am`（不带 infra），或者先用临时 default 实现压住 infra 编译；推荐路径——继续走 Task 4 让所有 provider 至少先 stub 占住接口。
- [x] **不**做 `mvn install`——等 Task 4 之后整模块一起 install。
- [x] commit：`feat(application): extend diagnostics service & provider interface for new capabilities`
...
- [x] 创建 `AbstractDiagnosticsProvider.java` 作为 abstract class implements `DiagnosticsProvider`。包含：
...
- [x] 让 `MySqlDiagnosticsProvider`、`PostgreSqlDiagnosticsProvider`、`H2DiagnosticsProvider`、`OracleDiagnosticsProvider` 都继承 `AbstractDiagnosticsProvider`（构造函数把 translator 传给 super）；删除每个类内部已有的 `withDatabaseOverride` 私有方法。
- [x] 在 4 个 provider 里给所有新接口方法（`poolStatus` 替换原 `connectionPoolInfo`、`tableSpaceInfo` 新签名、4 个 mutation 方法）加 stub 实现，全部返回 `DiagnosticResult.unsupported(translator.get("diagnostics.<name>.unsupported.<engine>"))`。i18n key 暂时硬编码 fallback 字符串，Task 11 才补完整 properties 文件。
- [x] **注意**：旧 `lockInfo` / 旧 `connectionPoolInfo` / 旧 `tableSpaceInfo` 实现都要更新——因为它们的返回类型 record 变了。最简策略：暂时统一返回 `unsupported`，Task 5-9 再分别真实实现。
- [x] 编译验证：`cd server && mvn compile -q` 全模块零错。
- [x] commit：`refactor(infra): introduce AbstractDiagnosticsProvider; stub all new capability methods`
...
- [x] **RED**：在 `H2DiagnosticsProviderTest.java` 加测试用例：
...
- [x] 测试全 RED（除 capabilities 外，其它已是 stub 的 RED 是"reason 文案不匹配"或方法返回类型/调用对不上）。
- [x] **GREEN**：
...
- [x] 测试全 GREEN。
- [x] commit：`feat(infra/h2): implement table_space (partial); declare other diagnostics unsupported`
...
- [x] 创建 `OracleDiagnosticsProviderTest.java`（如尚无）。覆盖：
...
- [x] **GREEN**：在 `OracleDiagnosticsProvider`：
...
- [x] 测试 GREEN。
- [x] commit：`feat(infra/oracle): declare diagnostics & mutation explicitly unsupported (architecture pending)`
...
- [x] 在 `MySqlDiagnosticsProviderTest`（继承自 protected 模板方法）补测试：
...
- [x] 实现 `poolStatus`：4 次独立 query（避免 SHOW STATUS 不能 join），按 `Threads_connected - Threads_running` 算 `idleConnections`；`identifier = conn.host() + ":" + conn.port()`；权限不足走 `mapPermissionOrError`。
- [x] 实现 `tableSpaceInfo`：
...
- [x] `supportedCapabilities()` 加 `LOCK_INFO, POOL_STATUS, TABLE_SPACE`（保留 EXPLAIN, INDEX_HINTS）。
- [x] 测试 GREEN。

### 7.3 收尾

- [x] 跑 `mvn test -pl data-talk-infrastructure -Dtest=MySqlDiagnosticsProviderTest`。
- [x] commit：`feat(infra/mysql): implement lock_info, pool_status, table_space`
...
- [x] `supportedCapabilities()` 再加 `TERMINATE_SESSION, OPTIMIZE_TABLE`，最终 `Set.of(EXPLAIN, INDEX_HINTS, LOCK_INFO, POOL_STATUS, TABLE_SPACE, TERMINATE_SESSION, OPTIMIZE_TABLE)`。
- [x] 跑 `mvn test -pl data-talk-infrastructure -Dtest=MySqlDiagnosticsProviderTest`。
- [x] commit：`feat(infra/mysql): implement terminate_session, optimize_table mutations with two-phase preview`
...
- [x] 在 `PostgreSqlDiagnosticsProviderTest` 加测试用例（mirror Task 7 的形态，但用 PG 的 SQL）。
- [x] **GREEN** 实现 SQL（spec §6）：
...
- [x] 在 `PostgreSqlDiagnosticsProvider.supportedDriverTypes()` 确认含 `"postgres", "postgresql"` 双 alias（依据 `DATA_SOURCE_TYPE_COMPATIBILITY.md` §47）。
- [x] 权限不足：PG SQLState `42501` → `Unsupported(translator.get("diagnostics.error.permission_denied"))`；其它 SQLException → `DiagnosticError`。
- [x] `supportedCapabilities()` 加 `LOCK_INFO, POOL_STATUS, TABLE_SPACE`。
- [x] 测试 GREEN。
- [x] commit：`feat(infra/postgres): implement lock_info, pool_status, table_space`
...
- [x] `supportedCapabilities()` 加 `TERMINATE_SESSION, OPTIMIZE_TABLE`。
- [x] commit：`feat(infra/postgres): implement terminate_session, optimize_table mutations`
...
- [x] 在 `server/data-talk-adapter/src/main/resources/messages.properties` 加 24 个新 key（spec §11 表格"示例值（英）"列）。
- [x] 在 `messages_zh_CN.properties` 加同样 24 个 key（"示例值（中）"列）。
- [x] 把 Task 5-10 中暂用的 fallback 字符串全部替换成 `translator.get("diagnostics.<x>.<y>")` 调用（含模板参数 `{0}` `{1}` 用 `translator.get(key, args...)` 重载）。
- [x] 跑 `mvn test -pl data-talk-infrastructure`（H2 + 4 provider 全部通过）。
- [x] commit：`feat(i18n): add 24 diagnostics & mutation message keys (en + zh-CN)`

## Task 12: Application 层 install 收尾

- [x] `cd server && mvn install -pl data-talk-application -am -DskipTests`（`-am` 把 domain 也带上，确保 ~/.m2 全是新版本）。
- [x] `mvn install -pl data-talk-infrastructure -am -DskipTests`（同理）。
...
- [x] **RED** `LockInfoActionTest`（Mock `DiagnosticsService`）：
...
- [x] 同 13.1 形态——RED test → 删 stub → 注入 service → 三态序列化 → outputSchema 描述 9 个字段（scope, active, idle, max, threadsRunning, waiting, identifier, recommendations, unsupported/error）。
- [x] commit：`feat(adapter): wire LockInfoAction, PoolStatusAction to DiagnosticsService`

### 13.3 TableSpaceAction

- [x] 同 13.1，但 inputSchema 加 `tables: { type: array, items: { type: string }, optional }`。
- [x] handle() 接 `tables` 参数（缺省 null）传给 `diagnosticsService.tableSpaceInfo(sessionId, tables)`。
- [x] **输入校验（spec §7）**：每个 `tables[i]` 用正则 `^[a-zA-Z_][a-zA-Z0-9_]{0,63}$` 校验，不合规返 `{ error: { type: "INVALID_INPUT", message: i18n("diagnostics.error.invalid_table_name") } }`。
- [x] 测试 GREEN。
- [x] commit：`feat(adapter): wire TableSpaceAction with tables[] input validation`
...
- [x] **RED** `TerminateSessionConfirmableActionTest`（Mock `DiagnosticsService`）：
...
- [x] 测试 GREEN。
- [x] commit：`feat(adapter): add TerminateSessionConfirmableAction (L2 mutation, two-phase confirm)`

## Task 15: Adapter — OptimizeTableConfirmableAction (Batch 3a)

- [x] 同 Task 14 形态，input/output 按 spec §9：
...
- [x] commit：`feat(adapter): add OptimizeTableConfirmableAction (L2 mutation, two-phase confirm)`
...
- [x] 在 `AgentPromptContractTest` 加新断言（保持英文）：
...
- [x] 删除 `AGENTS.md` 第 118-119 行 "Not yet available" 段。
- [x] 在 §"Query Diagnostics" 段插入 `datatalk_lock_info`、`datatalk_pool_status`、`datatalk_table_space` 三条工具说明（spec §11 给的英文文案）。
- [x] 新增 §"Mutation Actions" 段，含 `datatalk_terminate_session`、`datatalk_optimize_table`（spec §11 给的英文）。
- [x] 在 §"Diagnostics Workflow Rules" 现有第 6 条后追加第 7-13 条（spec §11 给定）。
- [x] 跑 `mvn test -pl data-talk-adapter -Dtest=AgentPromptContractTest`，全 GREEN。
- [x] commit：`feat(prompt): document diagnostics & mutation actions in runtime AGENTS.md`
...
- [x] 创建 `DiagnosticsClosedLoopIT.java`（`@SpringBootTest`，使用 Mockito 替换 `DiagnosticsProvider` 注入桩 provider）。
- [x] **场景 1：lock → terminate**：
...
- [x] commit：`test(adapter): closed-loop diagnostics→mutation integration tests`

## Task 18: Final Verification (Batch 4)

- [x] `cd server && mvn clean verify` 全模块全测试（含 application 层 + infra 单测 + adapter IT）。
- [x] 手工 smoke：
...
- [x] commit：`chore(verify): run mvn verify and manual smoke for diagnostics & mutations`（如无 source 改动，省略此 commit）。

## Task 19: Documentation Housekeeping (Batch 4 收尾)

> CLAUDE.md "Post-Execution Document Housekeeping" 强制要求：plan 完成后必须做这些。

- [x] 把本计划文件中的所有 `- [ ]` 改成 `- [x]`，加 `## Status Notes` 章节列出实际偏差（参考其他完成 plan 的 Status Notes 风格）。
- [x] `docs/exec-plans/index.md`：把本计划从「活跃计划」表移到「已完成计划」表，加完成日期与简要总结。
- [x] `docs/product-specs/index.md`：把 Diagnostics & Mutation Design 从 "Active" 段移到 Completed 段（如果索引按状态分段）；如果索引仅是单表，更新摘要无需移动（已是单表，跳过 move 步骤，确保摘要准确）。
- [x] `docs/product-specs/2026-04-29-diagnostics-mutation-design.md`：顶部 `状态: Active` 改 `状态: Completed`。
- [x] `~/.data-talk/opencode/datatalk-tools-test-report.md`：把 §7 "不可用工具" 段重写——3 个工具不再 unsupported（除 H2 / Oracle 引擎外），新增 2 个 mutation 工具到测试覆盖；`成功率统计` 调整为 26 个真实工具 + 0 个 stub。
- [x] commit：`docs(housekeeping): mark diagnostics & mutation plan completed; sync indexes & test report`
- [x] push：把所有 commit 推到 remote develop 分支。

---

## Status Notes

- 执行模式按用户指令改为“先写完全部代码与测试，最后统一联调”，未采用本计划原始的 per-task RED → GREEN → commit 循环。
- 跳过 per-task `mvn compile` / `mvn test` / `mvn install`。Task 12 的 install 步骤未执行；联调修复阶段仅用 `-pl <module> -am` 定向测试提速，未依赖 `~/.m2` 旧 jar。
- 最终验证使用 `JAVA_HOME="$JAVA_21_HOME" PATH="$JAVA_21_HOME/bin:$PATH" mvn clean verify`，全模块通过。默认 shell Java 为 8，必须显式切到 Java 21。
- 手工 `spring-boot:run` smoke 按用户指令 deferred 到用户验收，未执行。
- 实际偏差：`JdbcUrlBuilder` 的 MySQL 无 database URL 调整为保留尾部 `/`，以保持现有连接 URL 契约；为满足英文 runtime prompt contract，移除了 AGENTS.md 中残留的非英文示例；全量 verify 暴露并修复了既有 controller/test fixture 问题和 `FlywayMigrationIT` 的 migration SQL 拆分问题。
- 数据源兼容性：未新增 datasource type；MySQL / PostgreSQL / H2 / Oracle 的新能力状态与本 spec 保持一致，`docs/DATA_SOURCE_TYPE_COMPATIBILITY.md` 无需新增条目。
- Push 按用户指令未执行，等待用户确认后再推送。
