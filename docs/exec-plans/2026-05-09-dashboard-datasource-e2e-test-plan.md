...
- [x] Edit `client/playwright.config.ts`.
- [x] Add or confirm `outputDir: '../tmp/playwright/test-results'`.
- [x] Change HTML reporter output to repo-root tmp:

  ```ts
  reporter: [['list'], ['html', { outputFolder: '../tmp/playwright/report', open: 'never' }]],
  ```

- [x] Keep the current evidence policy unless a failing workflow requires a documented change:
...
- [x] Verify generated paths are relative to `client/` and resolve to `/home/wushengzhou/workspace/github/data-talk/tmp/playwright/...`.
- [x] Run:
...
- [x] Create `client/tests/e2e/preflight.spec.ts`.
- [x] Create `client/tests/e2e/fixtures/preflight-report.ts` with:
...
- [x] Implement a report writer that writes Markdown to:
...
- [x] Add Playwright tests with tags in the titles:
...
- [x] Keep `@preflight-api` independent from the browser dev server.
- [x] Keep `@preflight-e2e` scoped to browser E2E readiness only.
- [x] Do not put these checks in global `beforeAll`; they must be independently runnable with `--grep @preflight`.
- [x] Run:
...
- [x] Create `client/tests/e2e/fixtures/dashboard-fixtures.ts`.
- [x] Export a valid dashboard factory:
...
- [x] Export an oversized dashboard factory that exceeds `256 * 1024` bytes to exercise `DashboardArtifactService.MAX_PAYLOAD_BYTES`.
- [x] Extend `client/tests/e2e/fixtures/adapter-client.ts` with typed helpers:
...
- [x] If file artifact REST endpoints are needed by later suites, add typed helpers only after confirming actual adapter routes:
...
- [x] Treat a failure inside `adapterClient` itself as test infrastructure failure, not product failure, unless the HTTP response proves a server behavior defect.
...
- [x] Create `client/tests/e2e/dashboard-api-contract.spec.ts`.
- [x] Use `adapterClient` plus `makeDashboardPayload`.
- [x] Add contract cases:
...
- [x] Do not assume missing `baseVersion` returns `400`; the current Java request has primitive `int baseVersion`, so the implementation may bind `0` and return conflict. The test must assert the actual contract after confirming the response body.
- [x] Add `@contract` and `@dashboard` tags to titles.
- [x] Run:
...
- [x] Create `client/tests/e2e/types/datatalk-e2e.d.ts`.
- [x] Declare the existing global surface plus dashboard extension:
...
- [x] Update `client/src/main.tsx` to expose:

  ```ts
  dashboard: () => useDashboardTabsStore.getState(),
  ```

- [x] Import `useDashboardTabsStore` from the existing dashboard store.
- [x] Prefer read-only store inspection in E2E tests. Add mutation helpers only when a browser workflow cannot seed state through UI/API.
- [x] Run:
...
- [x] Create `client/tests/e2e/pom/dashboard.page.ts`.
- [x] Include locators based on stable roles/test ids:
...
- [x] If the dashboard canvas lacks a stable test id, add `data-testid="dashboard-canvas"` to the smallest relevant container in `client/src/features/dashboard/dashboard-canvas.tsx`.
- [x] Avoid `waitForTimeout`; use `expect(locator).toBeVisible()` and `expect.poll` for store state.
- [x] Keep text locators constrained to fixed test language. When using `getByText`, assert the app/test fixture language is fixed and use exact text.
...
- [x] Replace the current skeleton content in `client/tests/e2e/dashboard.spec.ts` or move it into `client/tests/e2e/dashboard-ui.spec.ts` and delete the empty skeleton.
- [x] Build the fixture by rendering or routing to a chat message containing the dashboard block payload already supported by `dashboard-block.tsx`.
- [x] Cover the following cases with `@e2e @dashboard` tags:
...
- [x] Use `window.__DT_E2E__!.dashboard()` to assert tab count and active tab after promotion.
- [x] Use `window.__DT_E2E__!.stage()` to assert Stage panel activation if the store exposes a stable field.
- [x] Run:
...
- [x] Update `client/tests/e2e/pom/connection-manager.page.ts` rather than replacing it.
- [x] Remove silent no-op methods. When a required button/input is missing, the method must fail with a meaningful Playwright assertion.
- [x] Replace `waitForTimeout` with role/test-id assertions and `expect.poll` where state polling is required.
- [x] Add methods for existing and new data-source form controls:
...
- [x] Create `client/tests/e2e/pom/data-sources.page.ts` only if the new wrapper delegates to `connection-manager.page.ts` and keeps the existing methods available.
- [x] Preserve existing callers by adding aliases instead of renaming public methods without migration.
...
- [x] Add `client/src/features/settings/data-sources/__tests__/oceanbase-connection-form.test.tsx`.
- [x] Add `client/src/features/settings/data-sources/__tests__/dameng-connection-form.test.tsx`.
- [x] Add `client/src/features/settings/data-sources/__tests__/multi-mode-connection-fields.test.tsx`.
- [x] Extend existing `tidb-connection-form.test.tsx` only for TiDB regressions.
- [x] Assert OceanBase MySQL-mode behavior:
...
- [x] Assert `MultiModeConnectionFields` accessibility:
...
- [x] Create `client/tests/e2e/data-source-forms.spec.ts`.
- [x] Use `connection-manager.page.ts` or the delegating `data-sources.page.ts`.
- [x] Add browser cases with `@e2e @datasource` tags:
...
- [x] Intercept create/update requests with Playwright `page.route` when the test only needs frontend payload verification.
- [x] Use real adapter calls only in smoke tests that require backend persistence.
- [x] Run:
...
- [x] Extend `client/src/features/stage/components/__tests__/file-artifact-status-badge.test.tsx`.
- [x] For the current phase, assert the current emoji rendering behavior in `file-artifact-status-badge.tsx`.
- [x] Add a separate note in the test description or plan comments that replacing emoji with lucide/SVG icons is a visual cleanup task, not part of this E2E implementation.
- [x] Extend `client/src/features/stage/components/__tests__/files-library-tab.test.tsx` with:
...
- [x] Extend `client/src/features/dashboard/__tests__/dashboard-canvas.test.tsx` with:
...
- [x] Create `client/tests/e2e/visual-a11y.spec.ts`.
- [x] Cover `@visual @a11y` cases for:
...
- [x] Assert layout integrity with Playwright locators before screenshots:
...
- [x] Keep screenshots under `../tmp/playwright/test-results` through Playwright output config.
- [x] If axe-core is already available in the repo, use it. If not, use role/name/focus assertions without adding a new dependency unless the implementation owner approves the package change.
- [x] Run:
...
- [x] Create or extend `client/tests/e2e/file-artifacts.spec.ts` only if no suitable existing file covers the workflow.
- [x] Use dashboard API promotion to create a dashboard-backed file artifact when backend endpoints expose it.
- [x] Cover `@e2e @files @dashboard` cases:
...
- [x] If the adapter has no browser-consumable file artifact route for a case, mark that case `test.fixme` with the exact missing route and add the route to implementation notes.
- [x] Do not create fake file state through undocumented store mutation when a REST contract exists.
...
- [x] Create `client/tests/e2e/real-db-smoke.spec.ts`.
- [x] Add `@realdb @datasource` tags.
- [x] Use environment variables for externally managed services:
...
- [x] Skip each database case with `test.skip` when the required environment variable is absent.
- [x] For Dameng and OceanBase, prefer official or maintained Testcontainers images only when available to the implementation environment. If no reliable image is available, keep those cases skipped with a clear annotation.
- [x] Cover smoke behavior only:
...
- [x] Do not make real database smoke part of default local Playwright runs.
...
- [x] Document the final tags in the plan implementation notes or a test README near `client/tests/e2e`.
- [x] Ensure these tags are present:
...
- [x] Keep `@realdb` excluded from default CI unless the environment provisions services explicitly.
...
- [x] During any Playwright execution, watch for product deviations:
...
- [x] For each product deviation, create a BUG file under `docs/bugs/` with status `open`.
- [x] Register the BUG in `docs/bugs/index.md`.
- [x] Store only allowed archival screenshots under `docs/bugs/assets/<BUG-ID>/` when needed; keep traces, HAR files, HTML reports, and large artifacts under `tmp/`.
- [x] In the final implementation report, state the exact number of BUGs found and registered. State `0` explicitly when none are found.

Expected result: E2E discoveries are tracked in canonical BUG docs, not only in chat.

### 17. Consolidated Verification

- [x] Run frontend type check after all client edits:
...
- [x] Run targeted Playwright suites:

  ```bash
  cd client && npx playwright test --grep "@preflight|@contract|@dashboard|@datasource|@files"
  ```

- [x] Run visual/accessibility suite:

  ```bash
  cd client && npx playwright test --grep "@visual|@a11y"
  ```

- [x] Run real DB smoke only when services are available:

  ```bash
  cd client && npx playwright test --grep @realdb
  ```

- [x] If backend code is edited to satisfy a discovered defect, run:
...
- [x] Mark completed checkboxes in this plan as implementation finishes.
- [x] Move this plan entry from Active to Completed in [docs/exec-plans/index.md](index.md) after execution finishes.
- [x] If the product spec is fully implemented, update [docs/product-specs/index.md](../product-specs/index.md) to reflect implementation status.
- [x] Propagate durable conventions to canonical docs when they become established:
