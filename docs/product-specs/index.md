...
**定位**：砍掉传统 DBA 运维功能（权限管理/备份恢复/跨库迁移/存储过程），聚焦 AI 驱动的数据流通与智能报告。

**关键能力**：AI 文件上传与智能识别（CSV/Excel/JSON/SQL 拖拽/粘贴 → AI 分析 → 导入/分析/报告路由）、对话式数据导入导出（一句话导出表/查询结果，一句话导入文件建表）、外部数据采集（Skill 驱动的 HTTP/REST/GraphQL 通用脚手架 + 电商平台垂直 skill）、智能报告生成（基于对话/查询/Dashboard 生成 PDF/HTML/Markdown 报告）。

详细路线图见 [Phase 3 Roadmap](../exec-plans/2026-05-12-phase-3-roadmap-plan.md)。
...
| 设计文档 | 日期 | 主题 |
|----------|------|------|
| [SQL 编辑器固定 Session 上下文持久化](./2026-05-13-sql-editor-session-context-persistence-design.md) | 2026-05-13 | 工作台 SQL 编辑器手动选定的执行上下文（连接 / 数据库 / Schema）随 Tab 持久化到后端，页面刷新后自动恢复 |
| [用户时区配置与时间格式化](./2026-05-13-timezone-settings-design.md) | 2026-05-13 | 设置通用页新增时区选择器，SQL 查询结果中时间类型字段按用户时区格式化展示，中国默认 `yyyy-MM-dd HH:mm:ss` |
| [External Data Ingestion via Skills — Generic HTTP Scaffolding Design](./2026-05-12-external-data-ingestion-skills-design.md) | 2026-05-12 | **Shipped.** Task 10 三期首份 child spec：通用 HTTP / REST / JSONL / CSV / 静态 HTML `<table>` 采集 scaffolding。架构纵切片：6 个新 MCP tool（`datatalk_http_request` / `_infer_ingestion_schema` / `_create_ingestion_table` / `_ingest_payload` / `_get_ingestion_job` / `_list_ingestion_jobs`），2 个新 SQLite 表（V20 `ingestion_credential` + `ingestion_job` 9 态状态机），扩 `file_artifact.kind` CHECK 加 `ingestion_payload`（external-registered 落 `~/.data-talk/ingestion/<jobId>/payload.<ext>`），2 个新 Stage Tab type（`ingestion_job` + `ingestion_library`，均 workspace scope），8 个新 DtEvent，独立 `data-ingestion` skill 包仿 bezel 路径打包入 classpath + 启动期 `ensureDataIngestionSkill` 解压幂等。HTTP fetch 双轨：默认走 `datatalk_http_request` 服务端 MCP（含 SSRF deny list 与 500MB 上限），skill 可显式声明 bash fallback。Day-1 auth scheme 4 种：None / Bearer / API Key (header 或 query) / Basic（SecretVault.seal 加密）。Day-1 分页 3 型：page / offset / cursor。Day-1 写入 dialect 4 个：mysql / postgresql / h2 / sqlite，统一 `IngestionDdlAdapter` 抽象；其它 15 个 first-class kind 返回 `dialect_unsupported` 每 kind 一份 follow-up child plan。L2 二期风险：Tab 内 inline confirm + `IngestionConfirmedToken` 5 分钟单次内存 token + 服务端 `SqlStatementGuard.classifyAndAudit(ingestionContext)` 仍走分类 + audit 不绕过。审计 fan-out：`ingestion_job` 表 + `file_artifact` 行 + 8 DtEvent + guard audit 行覆盖 §3.8 三项要求。控件五态 token 映射在 §6.3 完整列（input/select/checkbox/primary/secondary button/mapping row/password input/phase stepper dot），mapping 行 `skip=true` 用 `bg.subtle + text.muted + line-through`（不走 `interaction.selected`）。Settings → Credentials sub-page Day-1 直接做，4 scheme 条件渲染 + 被引用 409 + SET NULL 清理。6 Phase 串行：P1 DB+Domain+Credential / P2 Fetcher+SSRF / P3 Inferrer / P4 前端 Tab / P5 DDL Adapter+Executor+L2 / P6 Skill bundle+AGENTS+E2E。Out-of-Scope 14 项明确：平台特定 skill / OAuth2-HMAC-mTLS / JS-rendered 页 / Excel-XML-TSV-Parquet / 非 4 dialect 写入 / 增量同步 / 定时调度 / Staging table atomic swap / ALTER 已存在表 / 加密落盘 / 团队 vault / Webhook 触发 / 流式直写超 500MB / per-credential rate limit |
| [Data Source Coverage: GaussDB Design](./2026-05-12-data-source-coverage-gaussdb-design.md) | 2026-05-12 | Wave C step 6 child design：`gaussdb` canonical kind，集中式密码连接，driver `com.huaweicloud:gaussdbjdbc:v2.0-8.218.0`（Maven Central），URL `jdbc:postgresql://host:8000/database`，默认端口 8000；消费 `PgForkReuseRule` 6 抽象基类 + 6 GaussDB `*ReuseIT` 子类；PG splitter/metadata/batch DML 复用；12 项 system schema filter；5 条 GaussDB 专属 L3 risk rules；全 9 hooks dialect_unsupported diagnostics；前端 PG-fork 复用模式 connection form + picker + i18n；MCP ConnectionObjectType enum + AGENTS.md。Intentionally unsupported Day-1：分布式/DWS/SSL/Kerberos/EXPLAIN/ER DDL |
| [bezel — Premium Industrial Dashboard Skill 设计](./2026-05-11-bezel-skill-design.md) | 2026-05-11 | 把 `tmp/Dashboard/` 12 张 premium HTML 沉淀为独立 GitHub repo `wallfacers/bezel`（letterpress 形态：`skills/bezel/SKILL.md` + `.claude-plugin/marketplace.json`），作为 AI 看的「设计语言 + 12 行业 pattern + JSON→HTML 编译契约 + 数据接口契约」参考。data-talk dashboard 渲染管线大改：JSON（source-of-truth，schema 升 v2 加 `theme`/`patternId`/`refresh`/`renderer`，`layout.engine` 从 `'grid'` 改 `'free'`） → AI 一次性产出 dashboard.html artifact → 后端落盘双 artifact → 前端 `dashboard-tab.tsx` 重写为 iframe sandbox 宿主 → HTML 内置 polling 周期调 `POST /api/dashboards/{id}/widgets/{wid}/data` 拉真实数据 ECharts setOption（不重建 DOM）。现有 React `dashboard-canvas` + `chart-widget` + `grid-layout-engine` **彻底下线**。bezel 通过 server classpath 资源 + 启动期解压模式集成（与 `OpenCodeBinaryResolver.ensureNodeModules()` 同形态）：构建期 `scripts/sync-bezel.sh <tag>` vendor 进 `server/data-talk-infrastructure/src/main/resources/opencode/skills-src/bezel/` + maven-assembly-plugin 打 `bezel.tar.gz` 入 classpath；启动期 `OpenCodeBinaryResolver.ensureBezelSkill(projectRoot)` 解压到 `data-talk/.opencode/skills/bezel/`，marker `.bezel-installed` + 版本号比对实现幂等与升级；OpenCode 子进程 cwd = data-talk 仓库根自动读 SKILL.md。CSP `sandbox="allow-scripts"`（无 same-origin）+ `connect-src 'self'` 限 polling 只能调 data-talk 后端。后端新增 `GET /api/dashboards/{id}/html` + `POST /api/dashboards/{id}/widgets/{wid}/data` 两个 endpoint；widget SQL 仍走 `ParameterizedSqlExecutor` + `SqlStatementGuard` L1。v1→v2 lazy migration：theme 落 `industry-neutral`、patternId 由 `widget.type` 推断、layout.engine 改 `free`；HTML artifact 缺失时弹 modal 引导用户手工 chat 重生成视觉，不做后端自动重生成。明确 Design Inputs（iframe 外壳沿用 client/DESIGN.md cobalt/neutral token，iframe 内部 bezel 行业 theme 自包含为显式偏离）+ Data Source Type Compatibility Gate（widget SQL 复用现有所有 connection kind，零分支）。Open Risks：R1 Tauri webview CSP 与 ECharts CDN（备选内联 UMD ~250KB gzipped）/ R2 HTML XSS（bezel `scripts/validate.py` + server-side Java 二次校验）/ R3 BUG-0009 Files Library dashboard kind 渲染回归 / R4 polling QPS 上限（per-dashboardId 20 QPS rate limit + visibilitychange 暂停）/ R5 JSON↔HTML drift（HTML head 内嵌 `__JSON_HASH__` meta + 加载比对 banner）/ R6 bezel 解压失败 noop fallback / R7 v1 手工迁移 / R8 OpenCode vs Claude Code SKILL.md frontmatter 字段差异（仅用共同支持的 `name`+`description`，其余正文化）。Out of Scope 10 项（SSE/WebSocket、URL 公开分享、CRDT 协同、手编 HTML、chart fence 渲染替换、PDF/图片 export、第 13+ 行业、AI 自动重生 v1 HTML、ECharts UMD 离线默认化、.opencode 管理 UI）。验收 6 大类：bezel repo 可装、启动期 deploy 幂等+版本升级、chat 内生成 v2 可用、旧 React 完全删除（tsc/test/mvn 全绿）、v1 不破坏、E2E `dashboard-bezel-v2.spec.ts` 覆盖端到端 |
| [12 行业场景大屏独立精品化重设计](./2026-05-11-dashboard-premium-redesign-design.md) | 2026-05-11 | tmp/Dashboard 下 12 个行业大屏 HTML 的独立精品化升级：每屏独立美学方向、布局范式、数据增强策略，从 4-6 个图表提升至 8-10 个，数据注入真实业务波动与故事性 |
| [SQL 结果集全屏查看](./2026-05-10-sql-result-fullscreen-design.md) | 2026-05-10 | 在 SQL 编辑器结果集工具栏下载 CSV 按钮右侧新增全屏弹框按钮，点击后以 Portal 叠加层放大展示当前结果集表格（含工具栏和分页），复用 chart-expand-modal 弹框模式 |
| [Dashboard / FileArtifact + Data Source E2E Test Design](./2026-05-09-dashboard-datasource-e2e-test-design.md) | 2026-05-09 | 将 `tmp/e2e-playwright-testing-prompt.md` 迁移为正式 E2E 测试设计：覆盖 Dashboard-FileArtifact 集成与 Dameng/OceanBase 一等数据源支持。采用分层 suite 方案替代单个 mega prompt：Suite 0 preflight/readiness gate（实现为 `preflight.spec.ts` 中的 Playwright tests，使用 `@preflight` / `@preflight-api` / `@preflight-e2e`，端点检查按 suite 条件启用，可 `--grep @preflight` 单跑）、Suite 1 Dashboard/FileArtifact API contract（含 256KB payload cap 断言）、Suite 2 Dashboard 当前可达 UI（明确当前 `__DT_E2E__` 仅暴露 `stage/er/session/coordinator`，Dashboard seed 需补 `dashboard()` 或走产品路径；Playwright-facing 类型声明放 `client/tests/e2e/types/datatalk-e2e.d.ts`）、Suite 3 Dameng/OceanBase 连接表单（扩展现有 `connection-manager.page.ts`，先清 silent no-op / `waitForTimeout`）、Suite 4 Vitest component（标注现有 badge/files-library/data-sources/tidb/dashboard-canvas 测试为基线；当前 badge 断言 emoji，lucide 替换另立 cleanup）、Suite 5 visual/a11y/responsive、Suite 6 opt-in real DB smoke（OceanBase 可用 Testcontainers，Dameng 需显式 DM8 环境）。明确 Design Inputs、Data Source Type Compatibility Gate、BUG 登记与 `tmp/` 证据规则、Playwright locator/POM 规范（role/label 优先，`getByText` 仅固定 locale）、CI artifact 目标从当前 `client/playwright.config.ts` 基线收敛到 repo-root `../tmp/playwright/*` 且 `video: off`，以及 adapterClient/helper 失败按测试基础设施处理。 |
| [Dashboard ↔ File Artifact Integration Design](./2026-05-09-dashboard-file-artifact-integration-design.md) | 2026-05-09 | 修补 `2026-05-08-report-dashboard-p1-plan.md` Task B4 暴露的 file_artifact 集成断层（plan 假设的 `FileArtifactService.create / readBytes / replaceBytes` 与 `FileArtifactKind.DASHBOARD` 在当前代码中均不存在）：通过外部登记型 artifact（External-Registered Artifact）方案让 Dashboard 真正进入 file_artifact 表 + 生命周期管理体系，闭合 5 个真实缺口（`FileArtifactKind.DASHBOARD` 死代码 / `ConnectionDeletionService` 不清理 dashboard 孤儿 / Files Library 看不见 dashboard / housekeeping 失效 / 前端 TS `FileArtifactKind` 联合类型缺 `'dashboard'`；同时澄清 FTS 不属本范围，FTS 索引在 `stage_tab_index` 不在 `file_artifact`）。3 不破：spec §10.1 dashboard 物理路径**不**绑 connection 目录、§10.5 connection 删除两阶段、§11.3 ui_patch 原子性 + baseVersion 校验基于持久化态。方案对比 A/B/C 后选 A（保 §10.1 + 改动可控 + 复用价值），否决 B（PhysicalMover/Reconciler/SessionWorkdirRoot 三处分支放大测试面）与 C（推翻 §10.1 hybrid widget override 决策）。5 决策点对齐：**D1** external 标记落 `file_artifact.external` BOOLEAN 列（非 metadata_json）+ kind CHECK rebuild 合并为 V15 整表 migration（边际成本近零，可索引可加约束，未来扩展为 `path_owner` 路径清晰）；**D2** `DashboardArtifactService` 自管 `~/.data-talk/dashboards/` atomic write（temp + fsync + rename + fsync parent），后调 `FileArtifactService.registerExternal` 仅登记 DB（FileArtifactService 不踏 external 路径，模块边界清晰）；**D3** dashboard 行 `status='archived'` 复用现有枚举，**重新校准 archived 语义**——不再等价 frozen / read-only，按 kind 决定是否允许 `replaceBytesAtomic` 改写（dashboard 允许、其它 5 类保持 frozen）；`detachArchivedFromConnection` 现有 SQL（`status='archived' AND scope='workspace'`）天然覆盖 dashboard，零代码改动；**D4** patch 同步落盘，频率由前端 `dashboard-tabs-store` debounce 250ms 控制（C3 baseVersion 校验必须基于持久化态，禁止后端 buffer/coalescing/WAL）；**D5** `FileArtifactReconciler` 新增 `reconcileExternalDirs` 方法在 `runFullReconcile` 末尾扫 `dashboardsRoot()`，**双向覆盖**孤儿：盘有 × DB 无 → 删盘 + DB 有（external=1）× 盘没 → deleteById + publishDiscarded；`reconcileWorkspacesTree` 现有 archived 物理存在性校验对 external 行旁路。3 个新 API（`registerExternal(id, kind, scope, connectionId, sessionId, absolutePath, title, summary, metadata)` 不传 sizeBytes 由 `Files.size(path)` 自读避免 caller 传错；`readBytes(artifactId)` external 行文件不存在抛 `ArtifactNotFoundException` 映射 HTTP 404 与 managed 行一致；`replaceBytesAtomic(artifactId, bytes)` external 行先 `Files.createDirectories(parent)` 再 atomic write + UPDATE size_bytes/updated_at，不动 status/connection_id/kind/external）。`SessionWorkdirRoot` 新加 `dashboardsRoot()`（绝对路径 `System.getProperty("user.home") + "/.data-talk/dashboards/"`）+ `externalManagedRoots()`（当前仅 dashboardsRoot，未来扩）。`ConnectionDeletionService` 零改动 verified（`JdbcFileArtifactRepository.detachArchivedFromConnection` 行 245-267 仅 `UPDATE connection_id=NULL + metadata_json` 合并 orphan 字段，**不触碰物理文件**，external 绝对路径天然兼容）。`FrontmatterParser` 不动（external 行 JSON 不走 frontmatter，元数据由 caller 显式传入）。`ArtifactWatcherService` 不动（仍只监听 sessions/）。前端**必须**补 2 行（`file-artifacts.ts` `FileArtifactKind` 联合类型加 `'dashboard'` + `EMPTY_KIND_GROUPS` 加 dashboard 条目）——纠正大纲早期"前端零改动"误判。V15 migration 完整 SQL：CREATE TABLE `file_artifact_new` (新加 `external INTEGER NOT NULL DEFAULT 0 CHECK(external IN (0,1))` + kind CHECK 加 `'dashboard'`) → `INSERT ... SELECT` 全字段 backfill external=0 → `DROP TABLE` → `RENAME TO` → 重建 4 索引（含新加 `idx_file_artifact_external WHERE external=1`）。Rollback 路径明确：down migration 前必须先导出/删除所有 `kind='dashboard' OR external=1` 的行，含手工 helper `migrate-rollback-helper.sql`。3 task 分解（A1 V15 + Kind enum + FileArtifact.external 字段 + Repository 扩展；A2 3 新 API + Reconciler 旁路 + dashboardsRoot + status 语义校准代码审计；A3 前端 2 行）；A2 完成即解除 P1 plan B4 BLOCKED。Out of Scope：不引入 path_owner 三态枚举（YAGNI）/ 不引入 WAL or batched writes / 不扩展第 7 种 FileArtifactKind / 不动 ArtifactWatcherService / 不重构 FrontmatterParser / 不抽 AtomicFileWriter 公共类（dashboard 唯一外部 atomic write 场景，复用门槛未到）/ 不实现 dashboard discarded lifecycle / 不动 archiveCandidate/archive/discard 三套主流程。 |
...
| [Data Source Coverage: KingbaseES Design](./2026-05-08-data-source-coverage-kingbase-design.md) | 2026-05-08 | Wave C step 4 child design（最后一份）：以 `kingbase` 为 canonical kind，**唯一允许的 wave-c alias 归一**：`kingbasees` → `kingbase`（umbrella §8 line 293 例外），其他变体（`kb` / `金仓` / `人大金仓`）一律拒绝；Day-1 PG-mode first-class only，Oracle-mode `dialect_unsupported`（独立 Day-3 plan）；driver `cn.com.kingbase:kingbase8:9.0.x`（商业，二次分发权 = 无；Maven Central 直拉 + vendor portal documentation fallback；offline jar 入仓违反 umbrella §10 终止条件）；URL `jdbc:kingbase8://<host>:<port>/<database>` 默认端口 54321（与 PG 5432 不同，与 KingbaseES V8 server 默认一致）；database 字段必填（PG-required 模式）；KingbaseES V8 系列专属，V7 / V9 推 Day-3；零新增 ConnectionRecord 列（复用 oceanbase Flyway V18 `compatibility_mode` 共享列）；零新 Flyway 迁移；**消费两个上游套件**（wave-c 唯一同时消费两套件的 kind）：(1) `PgForkReuseRule` from opengauss（commit c58b7fd）= 6 concrete `Kingbase*ReuseIT` 子类继承 `AbstractPgFork*ReuseTest` 闭环 opengauss → kingbase 复用；(2) `MultiModeConnectionShape` v1 from oceanbase（commit 1231153）= 纯追加 kingbase 行（pg / oracle 合法，PG-mode Day-1 first-class），零 v1 函数签名修改；前端复用 oceanbase ship 的 `multi-mode-connection-fields.tsx`（modeOptions=["pg","oracle"], modeDisabled=["oracle"]）；splitter 复用 `PostgresJdbcSqlStatementSplitter`，等价测试 PG 6 case + KingbaseES 增量 2 case（PG-style DO 块 hit / Oracle-style 块由 risk Channel 2 拒绝）；metadata 复用 PG 分支同等 SQL（`pg_namespace` / `pg_class` / `pg_attribute` / `pg_index`），SYS_* 视图 Day-1 不触碰（Oracle-mode Day-3 才考虑）；6 项 system schema 过滤（4 PG + 2 KingbaseES 增量 sys / sys_catalog）；risk classifier **双通道**：通道 1 = 4 个 anchored L3（SYS_TABLE_DDL / SYS_ADMIN_SCHEMA_DDL / SYS_KILL / FLASHBACK）聚合 `kingbase_admin_command` label；通道 2 = 2 个 anchored `dialect_unsupported`（KB_BACKUP_RESTORE CLI 必须 \s+ 后跟随 + ORACLE_PLSQL_BLOCK），由 SqlExecuteService Stage 1 在 JDBC 调用前拦截，i18n key `risk.dialect_unsupported.kingbase.<reason>`，全 `^\s*` 起始锚 + `\b` 词边界（与 ad4c1f0 governance 同一规范）；`KingbaseDiagnosticsProvider` Day-1 全 9 hook（7 诊断 + 2 ER）`dialect_unsupported`，仅 `explain_real` + `index_hints` 进 day2 plan §Day-3 kingbase 行（已锚定 line 3119：复用 opengauss-shipped `PostgresJsonPlanParser` + B-tree + SYS_* 视图与 pg_* 同源处理），其余 5 项进独立 Day-3 spec；**零跨 kind reuse 抽象产出**（pure consumer pattern）；MCP `ConnectionObjectType` enum 加 `kingbase` + AGENTS.md KingbaseES 段（含 kingbasees alias 归一文案）在 child plan verify 后才进（§7.5 时机硬约束）；T2 fixture（manual smoke `tools/manual-smoke/kingbase-day1.sh` 9 case + JDBC mock 单元测试，**6 IT 与 opengauss 6 IT 不同：标 @Disabled 默认 CI 不跑**，通过 `-Dkingbase.it.enabled=true` profile 在本地 / QA / acceptance 阶段触发，与 dameng T2 同形）；显式 Out-of-Scope 11 项（Oracle-mode / Oracle-style PL/SQL block / KBBACKUP-KBRESTORE CLI / 9 诊断 + ER 全 unsupported / V7-V9 驱动 / KB_HA Repmgr 集群 / 企业版数据脱敏-TDE-多租户 / 离线 jar 入仓 / SYS_* 元数据 / CI 启动 KingbaseES container / KingbaseES Cloud） |
| [Data Source Coverage: Dameng Design](./2026-05-08-data-source-coverage-dameng-design.md) | 2026-05-08 | Wave C step 5 child design：以 `dameng` 为 canonical kind（无 alias，`dm` / `dm8` / `达梦` / `武汉达梦` 全部拒绝），单 mode（不消费 MultiModeConnectionShape / PgForkReuseRule），Day-1 deliverable = SELECT + L1 metadata + 风险分级 DML/DDL；driver `com.dameng:DmJdbcDriverX:8.1.x`（商业，二次分发权 = 无；Maven Central 直拉，禁止离线 jar 入仓 = 触发 umbrella §10 终止条件；DM 9 推 Day-3）；URL `jdbc:dm://<host>:<port>` 默认端口 5236，无 `/<database>` 后缀；`databaseName` 字段复用为初始 schema 名（Oracle-precedent；零新增 ConnectionRecord 列）；`?schema=` URL 参数 vs 连接后 `SET SCHEMA` 由 child plan Step 0 fixture 验证；splitter 复用 `GenericSqlStatementSplitter`（与 Oracle 同），不新建 `DamengSqlStatementSplitter`；metadata 复用 Oracle 分支同等 SQL（`ALL_TABLES` / `ALL_TAB_COLUMNS` / `ALL_INDEXES`），DM 8 Oracle 兼容 95%+；normalize 复用 Oracle baseline（CHAR 保留尾空格 / VARCHAR2 / NUMBER BigDecimal / DATE 含时分秒 / TIMESTAMP / CLOB-BLOB）；risk classifier **双通道**：通道 1 = 5 个 dameng L3 anchored pattern（TABLESPACE / USER / ROLE / GRANT-REVOKE / DROP-OBJECT）聚合 `dameng_admin_command` label；通道 2 = 3 个 anchored `dialect_unsupported` pattern（PLSQL_BLOCK / PROCEDURE_DDL / EXP_IMP，由 SqlExecuteService Stage 1 在 JDBC 调用前拦截，i18n key `risk.dialect_unsupported.dameng.<reason>`），全 `^\s*` 起始锚 + `\b` 词边界（与 ad4c1f0 governance 同一规范），EXP/IMP 必须后跟 `\s+` 防 `SELECT EXP(...)` 误判；5 项默认 system schema 过滤（SYS / SYSDBA / SYSAUDITOR / SYSSSO / CTISYS）；`DamengDiagnosticsProvider` Day-1 全 9 hook（7 诊断 + 2 ER）`dialect_unsupported`（与 umbrella §5 + opengauss / oceanbase 立场一致），仅 `explain_real` 进 day2 plan §Day-3 dameng 行（已锚定 line 3121 新建 `DamengTabularGrammar` 复用 `TabularLayout` + `TextPlanGrammar`），`index_hints` Day-2 显式延后（CHAR/VARCHAR2/NUMBER 类型规范化前置），其余 5 项进独立 Day-3 spec；**零跨 kind reuse 抽象产出**（YAGNI：dameng 是 wave-c 唯一 Oracle-like kind，单消费者抽象不立）；MCP `ConnectionObjectType` enum 加 `dameng` + AGENTS.md Dameng 段在 child plan verify 后才进（§7.5 时机硬约束）；T2 fixture（manual smoke `tools/manual-smoke/dameng-day1.sh` 9 case + JDBC mock 单元测试，CI 不跑 IT，与 kingbase 同形）；显式 Out-of-Scope 11 项（PL/SQL block 执行 / PROCEDURE-FUNCTION-TRIGGER-PACKAGE DDL / EXP-IMP 命令 / 9 诊断 + ER 全 unsupported / DM 9 / DataWatch DSC 集群 / 企业版地理空间-全文-XML / 离线 jar 入仓 / 自动 schema 切换 / CI 启动 Dameng container / Dameng Cloud） |
| [Data Source Coverage: OceanBase Design](./2026-05-08-data-source-coverage-oceanbase-design.md) | 2026-05-08 | Wave C step 3 child design：以 `oceanbase` 为 canonical kind（无 alias，`oceanbase-ce` / `ob` / `obcluster` 全部拒绝），Day-1 仅 MySQL-mode first-class，Oracle-mode 整体 `dialect_unsupported`；driver `com.oceanbase:oceanbase-client` 2.4.x 系列最新稳定 patch（Apache 2.0，Maven Central 直发，与 `mysql-connector-j` 同 classpath 共存，`acceptsURL` 协议路由），URL `jdbc:oceanbase://<host>:<port>/<database>` 默认端口 2881（OBServer 直连）/ 用户可改 2883（OBProxy）；username 走 ConnectionService 单一拼装点 `<user>@<tenant>[#<cluster>]`，结构化字段始终保留原值；新增 Flyway V18 三列：`compatibility_mode` 共享列 CHECK ('mysql','oracle','pg') / `oceanbase_tenant` kind-private + `(kind <> 'oceanbase' OR oceanbase_tenant IS NOT NULL)` CHECK 约束 / `oceanbase_cluster` 可选；产出 **`MultiModeConnectionShape`** 跨 kind 复用抽象单元（应用层中性命名，`compatibilityMode` 枚举 + `validateModeForKind` + `isDay1FirstClassMode`，kingbase 后续消费的硬门槛）；前端 `multi-mode-connection-fields.tsx` 条件渲染骨架（mode picker + dialect_unsupported 灰显 tooltip + tenant/cluster kind-private 字段）；splitter 复用 `MySqlSqlStatementSplitter` + `OceanBaseSplitterReuseIT extends AbstractMySqlSplitterEquivalenceTest`；6 个 OB 特有 anchored risk pattern（OUTLINE / TENANT / RESOURCE POOL UNIT / ALTER SYSTEM / MAJOR-MINOR FREEZE / BACKUP-RESTORE）全 `^\s*` 起始锚 + `\b` 词边界（与 ad4c1f0 governance 报告同一规范），统一聚合 `oceanbase_admin_command` risk label；6 concrete `OceanBase*ReuseIT` 子类继承 tidb-shipped abstract bases（`AbstractMySqlSplitterEquivalenceTest` / `AbstractMySqlMetadataReuseTest` / `AbstractMySqlTargetResolutionReuseTest` / `AbstractMySqlBatchDmlReuseTest` / `AbstractMySqlResultNormalizationReuseTest` / `AbstractMySqlConnectionTestReuseTest`）闭环 `tidb`→`oceanbase` 跨 kind 复用；7 项 system schema filter（mysql/information_schema/performance_schema/sys + oceanbase/LBACSYS/SYS）；`OceanBaseDiagnosticsProvider` Day-1 全 9 hook（7 诊断 + 2 ER）`dialect_unsupported`（与 umbrella §5 + opengauss step 2 立场一致），仅 `explain_real` / `index_hints` 进 day2 plan §Day-3 oceanbase 行（已锚定 line 3117 EXPLAIN FORMAT=JSON 复用 parseMySqlJsonPlan + MySQL-style B-tree），其余 5 项进独立 Day-3 spec；MCP `ConnectionObjectType` enum 加 `oceanbase` + AGENTS.md OceanBase 段在 child plan verify 后才进（§7.5 时机硬约束）；T1 fixture（`oceanbase/oceanbase-ce:4.2.1-lts` log-based wait `.*observer\s+is\s+ready.*` 180s 超时，单例 container 共享 6 IT）；显式 Out-of-Scope 12 项（Oracle-mode / multi-tenant 跨租户路由 / OBProxy 配置 UI / tenant 自动发现 / runtime mode 切换 / ER / 主备复制 UI / 集群扩缩容 / Outline UI / 其他 wave-c kind / 驱动版本动态升级 / OceanBase Cloud 托管） |
