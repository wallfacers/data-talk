...
public class InverseSqlGenerator {
    ...
    public static String generate(
        ...
        return switch (operation) {
            case "INSERT" -> generateInsertInverse(tableName, pkColumns, generatedKeys);
            case "UPDATE" -> generateUpdateInverse(tableName, pkColumns, beforeState);
            case "DELETE" -> generateDeleteInverse(tableName, beforeState);
            default -> throw new IllegalArgumentException("Unsupported operation: " + operation);
        };
    }

    private static String generateInsertInverse(String tableName, Set<String> pkColumns, List<Map<String, Object>> generatedKeys) {
        ...
        if (pkValues.size() == 1) {
            return "DELETE FROM " + quoteIdentifier(tableName) + " WHERE " + quoteIdentifier(pkCol) + " = " + pkValues.get(0);
        }
        return "DELETE FROM " + quoteIdentifier(tableName) + " WHERE " + quoteIdentifier(pkCol) + " IN (" + String.join(", ", pkValues) + ")";
    }

    private static String generateUpdateInverse(String tableName, Set<String> pkColumns, List<Map<String, Object>> beforeState) {
        ...
        for (Map<String, Object> row : beforeState) {
            ...
            for (Map.Entry<String, Object> entry : row.entrySet()) {
                ...
                if (pkColumns.contains(col)) {
                    pkConditions.add(quoteIdentifier(col) + " = " + formatValue(entry.getValue()));
                } else {
                    setClauses.add(quoteIdentifier(col) + " = " + formatValue(entry.getValue()));
                }
            }
            if (setClauses.isEmpty()) {
                continue;
            }
            statements.add("UPDATE " + quoteIdentifier(tableName) + " SET " + String.join(", ", setClauses) + " WHERE " + String.join(" AND ", pkConditions));
        }
        return String.join(";\n", statements);
    }
    ...
    private static String generateDeleteInverse(String tableName, List<Map<String, Object>> beforeState) {
        ...
        return "INSERT INTO " + quoteIdentifier(tableName) + " (" +
            columns.stream().map(InverseSqlGenerator::quoteIdentifier).collect(Collectors.joining(", ")) +
