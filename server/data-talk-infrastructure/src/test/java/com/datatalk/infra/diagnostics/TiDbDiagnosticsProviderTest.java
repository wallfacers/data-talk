...
class TiDbDiagnosticsProviderTest {
    ...
    private TiDbDiagnosticsProvider provider;

    @BeforeEach
    void setUp() {
        provider = new TiDbDiagnosticsProvider(translator());
    }
    ...
    void diagnosticsAndMutationsReturnStructuredUnsupported() {
        assertUnsupported(provider.explain("SELECT 1", null, null, null, null), "EXPLAIN not supported for dialect: tidb");
        assertUnsupported(provider.indexHints("SELECT 1", null, null, null), "Index hints not supported for dialect: tidb");
