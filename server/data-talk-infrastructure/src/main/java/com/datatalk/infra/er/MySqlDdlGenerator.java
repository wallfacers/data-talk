...
public class MySqlDdlGenerator implements ErDdlGenerator {
    ...
    public GenerateResult generate(ErSchemaDiff diff) {
        ...
        if (diff instanceof ErSchemaDiff.ConstraintAdded added && added.constraint() instanceof ErDesignerRelation relation) {
            return generated(addFk(relation));
        }
        ...
        if (diff instanceof ErSchemaDiff.ConstraintAdded added && added.constraint() instanceof ErSchemaDiff.UniqueConstraint unique) {
            return skipped("add_unique", unique.tableName(), null);
        }
    ...
    private String renderColumn(ErDesignerColumn column) {
        ...
        if (column.comment() != null) builder.append(" COMMENT '").append(column.comment().replace("'", "''")).append("'");
