...
`renderKind` 字段决定运行时调度器是否对该 widget 调用 `echarts.init()`,并把对应 widget `type` 同步注入到 `BezelWidgetConfig.type`(详见 `references/data-contract.md` §5)。`renderKind: 'chart'` 的 widget,其 `options` 字段必须是合法 ECharts option(含 `series`/`xAxis`/`yAxis` 等结构),编译期会被拷为 `BezelWidgetConfig.baseOption`,iframe 首屏即 `setOption(baseOption)`。`renderKind: 'html'` 的 widget 编译期产出完整 DOM,运行时调度器**禁止**对其 `echarts.init`。

| 工业级别 generic widget | widget.type | renderKind |
|---|---|---|
| `generic.kpi-tile` | `kpi` | `html` |
| `generic.echarts-card` | `chart` | `chart` |
| `generic.table` | `table` | `html` |
| `generic.markdown` | `markdown` | `html` |
| `generic.section-header` | `section` | `html` |
| `generic.divider` | `divider` | `html` |
| `generic.image` | `image` | `html` |
| `generic.filter-bar` | `filter` | `html` |

行业 widget(`<industry>.<chart-name>`,例如 `ecommerce.funnel-gradient`)默认 `renderKind: 'chart'`,除非在对应 `industries/<industry>.md` 中显式标注为 `html`。

### `generic.kpi-tile`
- Single metric card with value, label, optional delta arrow.
- **renderKind: `html`**(纯 HTML 渲染,**不**调用 `echarts.init`)。
- SQL convention: `label TEXT, value NUMERIC, delta TEXT, trend TEXT('up'|'down')`
- `refresh.intervalMs`: 5000 — 轮询通过 `applyHtmlData('kpi', el, rows)` 重写 `.value` / `.delta` / `.trend` 元素文本

### `generic.echarts-card`
- ECharts chart wrapped in a standard card with title bar.
- **renderKind: `chart`**。`widget.options` SHALL 是合法 ECharts option(至少含 `series` 或 `xAxis`+`yAxis` 一组);编译期拷为 `BezelWidgetConfig.baseOption`,iframe 首屏 `setOption(baseOption)`,轮询只 `setOption({ dataset: { source: rows } })`。
...
- `refresh.intervalMs`: 30000 — 轮询通过 `applyHtmlData('table', el, rows)` 重写 `<tbody>` 行
...
- **renderKind: `html`** (1px line)。
- No SQL.
- `refresh.intervalMs`: 0 (static)

### `generic.image`
- Image placeholder with URL source and optional caption.
- **renderKind: `html`**(`<img>` in a card)。
- No SQL.
- `refresh.intervalMs`: 0 (static)

### `generic.filter-bar`
- Horizontal parameter input bar (dropdowns, date pickers, text inputs).
- **renderKind: `html`**;emits filter values to parent dashboard via postMessage `params/update`.
