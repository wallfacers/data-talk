...
- Single charts emitted to chat MUST be a ```` ```chart ```` (or ```` ```chart:<artifactId> ````) fenced code block whose body is the **complete ECharts option JSON, including literal data values**. The fenced body is **never empty**. The optional `:<artifactId>` suffix is provenance metadata only (forwarded as `sourceArtifactId` on promote-to-stage) — it does **not** supply chart data and does **not** auto-render the source rowset; an empty body crashes the frontend `ChartBlock` with `JSON.parse('')`. Pick exactly one render path per chart: either emit one fenced block *with full body*, or call `datatalk_render_chart` and emit no fenced block — never both for the same chart, and never an empty fence. Full rules: see skill:charts-and-dashboards.
