...
- Charts shown in chat render in exactly one place: a ```` ```chart ```` (or ```` ```chart:<artifactId> ````) fenced code block (`ChartBlock`), whose body is the **complete ECharts option JSON, including literal data values**. The fenced body is **never empty** (an empty body crashes `ChartBlock` with `JSON.parse('')`). The optional `:<artifactId>` suffix is provenance metadata only (forwarded as `sourceArtifactId` on promote-to-stage) — it does **not** supply chart data and does **not** auto-render the source rowset. `datatalk_render_chart` does **not** render a chart inline (its tool card is a compact reference + "在工作台查看" eye); call it only when a persistent artifact is required, and pair it with a fenced block if the chart should also be visible in chat. Full rules: see skill:charts-and-dashboards.
...
| user asks for an enterprise-style **report document** for upward reporting — 周报 / 月报 / 季报 / 复盘 / postmortem / 汇报 / 总结 / weekly report / monthly report / quarterly review / incident postmortem / 业务月报 / 事故复盘 — that needs to be exported as PDF / Markdown / printed and emailed to a manager | skill:ledger |
...
- skill:ledger — Enterprise upward-reporting documents (业务月报 / 季度复盘 / 事故 postmortem / weekly / monthly / quarterly review). White-background serif print layout. JSON → HTML + PDF + Markdown via `datatalk_promote_report`. Strictly NOT a bezel-style dashboard.
