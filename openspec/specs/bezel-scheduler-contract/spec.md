...
契约同时约束:
- 服务端 `DashboardCompiler` 编译期确定 widget type(由 `pattern-catalog.yaml` 的 `renderKind` 推导)
- 编译期 `HtmlValidator`(Java regex fingerprint)守卫
- 运行时 `scheduler.js` IIFE 按 widget type 分流初始化与数据应用
