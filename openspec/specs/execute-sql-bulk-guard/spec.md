# execute-sql-bulk-guard Specification

## Purpose

定义 AI 路径下批量 SQL 执行的防护机制：通过 CallerKind 区分用户与 AI 发起方，对 AI 提交的大批量写入 SQL（大体积、多 INSERT、源自文件）进行拦截并引导至 `datatalk_import_data` 替代路径，同时对用户路径无条件放行。
