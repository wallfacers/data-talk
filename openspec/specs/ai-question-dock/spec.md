# ai-question-dock Specification

## Purpose

定义 DataTalk 桥接 OpenCode question 工具以实现人在环（human-in-the-loop）确认的契约：将 OpenCode 的 `question.asked` / `question.replied` / `question.rejected` 事件翻译为 `DtEvent` 并经 SSE 下发；提供 REST 代理端点列举挂起问题并提交回答/拒绝；客户端按 session 维护挂起问题集合，并在 composer 位置呈现 Question Dock 取代普通输入框，支持选项选择与自定义文本输入。
