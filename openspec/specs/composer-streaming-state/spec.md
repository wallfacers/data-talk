# composer-streaming-state Specification

## Purpose

定义 prompt-composer 发送/停止按钮的状态机契约：streaming 真值的唯一权威源是 OpenCode 服务端 `SessionStatus`，客户端在 mount / 订阅 SSE 之前通过 HTTP 一次性 reconcile，禁止把 streaming 标志持久化到客户端存储。本 spec 同步约束历史加载守卫（`shouldSkipReplace`）与 streaming 标志在 mount 阶段的解耦关系，确保任意刷新路径都不会因 streaming 持久化错误而卡死历史 replace。
