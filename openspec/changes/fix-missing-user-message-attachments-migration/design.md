...
`user_message_attachments` 是 BUG-0056 修复时新增的持久化表，参见 archived change `2026-05-17-user-bubble-attachments`。当时的做法是直接在 `V1__init.sql` 末尾追加表定义而非新增独立 `V29__...sql` 迁移。
...
- 老 DB 升级路径：首次启动自动应用 V29，建表 + 索引，HTTP 500 消失
- 新 DB 安装路径：V1 不再含该表定义，由 V29 创建，二者 checksum 在所有环境保持稳定
...
### Decision 1: 新增 V29 而非修改 V1 checksum

**Alternatives:**

a) 改 V1 checksum + 文档要求所有人 `flyway repair`：高摩擦，每个 dev 都得手动操作，CI 也要修
b) 新增 `V29__user_message_attachments.sql` 同时从 V1 删除该表：低摩擦，Flyway 自动应用 V29，新装 / 老环境都走同一路径 ✅

**Chosen: b)**

**Rationale:** Flyway 的设计哲学就是"版本号递增 + 永不修改已应用版本"。V29 路径让升级行为对 dev 完全透明，唯一额外成本是新 fresh DB 也会多一次 migration（< 10ms），可忽略。

### Decision 2: 同步从 V1 删除该表定义

**Alternatives:**

a) V1 保留、V29 用 `CREATE TABLE IF NOT EXISTS`：新装环境 V1 建表后 V29 是 no-op；checksum 仍跟历史 V1 不一致，不解决核心问题
b) V1 删除、V29 唯一建表点：所有环境的 V1 checksum 回到追加前的状态，跟最早期 V1 一致；新装走 V29 ✅
...
**Rationale:** BUG-0061 文档已提供 sqlite3 CLI workaround，部分 dev 可能已经手工建过表。V29 用 `CREATE TABLE IF NOT EXISTS` + `CREATE INDEX IF NOT EXISTS` 对这部分环境幂等，不会因表已存在而失败。
...
- **Risk**: 多个开发者并发修 V29 文件名冲突
  - **Mitigation**: V29 是已知 next 可用版本号（grep V[0-9]+ 验证），本 change 单文件单 PR 不会冲突
- **Trade-off**: 不引入自动 repair = 一次性手动成本；引入则需写 CLI 工具，超出本 change 范围

## Migration Plan

1. 新增 `V29__user_message_attachments.sql`
...
4. 验证：
   - fresh DB：删 `~/.data-talk/datatalk.db` 重启，确认 `flyway_schema_history` 含 V1 + V29 两行
   - 老 DB：保留现有 DB，确认启动后自动应用 V29，表创建成功；如报 V1 checksum 错，按 Step 3 手工修一次
