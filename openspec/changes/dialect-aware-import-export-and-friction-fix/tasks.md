...
- [ ] 3.1 改 `DataImportService.buildCreateTableSql` / `buildInsertSql`：
...
- [ ] 3.2 改 `ScriptDataWriteService.buildCreateTableSql` / `buildCreateTableFromColumns` / `buildInsertSql`：
  - 同样改为接收 kind 参数
  - `writeStream(...)` / `write(...)` / `batch(...)` 公开方法签名加 `kind`，由 controller / caller 取自 `ConnectionRecord` 透传
  - 删除私有 `quoteIdentifier(String)`
- [ ] 3.3 改 `DataExportService.exportSqlInsert` 路径：
...
- [ ] 3.4 改 `InverseSqlGenerator` 全部 4 个 public 方法（buildInverseForInsert / Update / Delete / Mixed）：
...
- [ ] 4.1 `DataImportServiceTest` 增加用例：MySQL 反引号 + 含保留字列名（select, order, group）+ 列名含反引号字符的转义
- [ ] 4.2 `ScriptDataWriteServiceTest`：cross-DB MySQL→MySQL roundtrip 验证 + PG→PG + SQLServer→SQLServer
- [ ] 4.3 `DataExportServiceTest`：MySQL 导出 .sql → 内容包含反引号断言 + 通过 `DataImportService` 回灌同库成功
- [ ] 4.4 `InverseSqlGeneratorTest`：含保留字 / 空格 / 引号字符列名 + 三种方言（MySQL / PG / SQLServer）+ kind=null fallback 不崩

## 5. ui_exec 错误自愈（client-side 改动）

- [ ] 5.1 在 `client/src/features/` 内定位 `query_editor.run_sql` / `apply_text_edits` / `set_context` / `format_sql` / `focus` 的 handler（搜 `object === 'query_editor'`）
- [ ] 5.2 为每个 handler 在"无活跃 query_editor tab"分支返回结构化 `{ error: { code: "no_active_query_editor", message, nextAction: { object: "workspace", action: "open", params: { type: "query_editor", title: "Untitled SQL" } } } }`
- [ ] 5.3 对 `er_inspector` / `er_designer` / `dashboard` 三类 object 也走同模式：`no_active_er_inspector` / `no_active_er_designer` / `no_active_dashboard`
- [ ] 5.4 `UiActionsTest`（adapter 层契约测试）增加断言：当 client 返回 `error.code` 形如 `no_active_*` 时，response payload 校验通过、不抛 schema 异常
- [ ] 5.5 frontend 单测（vitest）：mock dispatch 无活跃 tab 场景，断言返回的错误对象结构符合 spec

## 6. data-collection skill 加 DO NOT 段

- [ ] 6.1 修改 `server/data-talk-adapter/src/main/resources/skills/data-collection/SKILL.md`：在 `# Data Collection Skill` 标题之后、`## Tool Surface` 之前插入 `## ❗ DO NOT` 段（按 design.md D7 文字）
- [ ] 6.2 新建 `server/data-talk-adapter/src/test/java/com/datatalk/adapter/skills/DataCollectionSkillContractTest.java`：
  - 加载 classpath 资源 `skills/data-collection/SKILL.md`
  - 断言包含 "DO NOT" / "pymysql" / "psycopg2" / "/api/script-data/write"
- [ ] 6.3 确认 `server/.opencode/skills` 与 `server/data-talk-adapter/.opencode/skills` 是否需要同步（若是从 main resources 软链 / 复制而来，可能需要 rebuild）

## 7. 文档同步

- [ ] 7.1 更新 `docs/DATA_SOURCE_TYPE_COMPATIBILITY.md`：
  ...
  - 如有适用，更新 `Backend Compatibility Checklist` 加一行"Identifier quoting follows IdentifierQuoter"
- [ ] 7.2 把 BUG-0066 / BUG-0067 文档的 `fixCommit` 字段从 `pending` 替换为本 change 实际 commit hash，并在 `index.md` 同步行
- [ ] 7.3 在 proposal.md 的 `OPEN QUESTION D` 旁记录用户最终决定（拒绝 / 透传）

## 8. 验收 / 端到端

- [ ] 8.1 `cd server && mvn install -pl data-talk-application -am -DskipTests`（因为改了 application 层，run 模式才看得到）
- [ ] 8.2 `cd server && mvn clean verify`：全测试套件绿
- [ ] 8.3 `cd client && npx tsc --noEmit`：前端零类型错误
- [ ] 8.4 `cd client && npm run test --run -- query_editor`：vitest 5.5 用例通过
