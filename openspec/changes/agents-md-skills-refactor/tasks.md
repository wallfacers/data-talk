...
- [ ] 7.2a / 7.2b / 7.2c **DEFERRED（本会话不执行）**：实际跑 E2E 需要 backend + frontend 长时间运行 + `DATATALK_REAL_OPENCODE_MODEL` 环境变量 + 真实 OpenCode model 调用。运行命令组：`cd server && SPRING_PROFILES_ACTIVE=e2e mvn spring-boot:run -pl data-talk-adapter` & `cd client && npm run dev` & `cd client && DATATALK_REAL_OPENCODE_MODEL=<model> npx playwright test agents-skills-regression.spec.ts`。结构性正确性已由 Phase 6 的 35 个 JUnit 测试（SkillRoutingContractTest + AgentPromptBuilderPlaceholderTest + AgentsTemplateContractTest + AgentPromptContractTest）保障；E2E 是行为面验证，跑通后回填本任务。
- [ ] 7.3 **DEFERRED（跟随 7.2）**：5 场景跑完按 BUG Gate 入档偏差；运行后无论 N=0 与否都在 PR 描述 / 最终响应明示"本次 E2E 共发现 N 个 BUG"。
...
- [ ] 8.3 **DEFERRED**：在 develop 上回归 backend 启动一次，确认日志中出现 13 条 `Synced skill '<name>' (N files) to ...`。本会话未跑（`mvn spring-boot:run` 是长任务）。建议在 PR 合入前手动跑一次：`cd server && mvn install -pl data-talk-adapter -am -DskipTests && mvn spring-boot:run -pl data-talk-adapter`，启动后立即 grep `Synced skill` 看日志含 13 条（bezel / data-ingestion + 11 新 skill），然后 Ctrl-C。
- [ ] 8.4 **DEFERRED**：`/opsx:archive agents-md-skills-refactor` 在以下三件事完成后再执行：(a) 8.3 backend 启动回归通过；(b) E2E 5 场景（7.2/7.3）跑通；(c) BUG-0045 关联 WIP 已合入。届时 archive 会把 `specs/agent-skill-routing/spec.md` 并入 `openspec/specs/`，变更目录搬迁至 `openspec/changes/archive/YYYY-MM-DD-agents-md-skills-refactor/`。
