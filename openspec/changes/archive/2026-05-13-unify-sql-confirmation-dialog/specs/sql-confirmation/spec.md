...
4. L3 仅当 `level === 'L3'` 时额外渲染 `sqlConfirmation.l3.irreversible`，使用 `status.danger` 强调色 + `font-semibold`。
...
#### Scenario: L3 在 L2 主体之上额外渲染不可撤销提示

- **GIVEN** `risk = { level: 'L3', reason: 'drop_table', affectedObjects: ['temp_log'] }`、`sqlPreview = "DROP TABLE temp_log"`
- **WHEN** 对话框打开
- **THEN** 内容区 SHALL 包含 SQL 文本、"受影响对象"标签、列表项 `temp_log`、描述段"将永久影响 temp_log。"、不可撤销提示"此操作无法撤销。"
