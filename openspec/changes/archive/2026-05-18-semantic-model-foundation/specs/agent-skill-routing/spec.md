...
- `{{SEMANTIC_MODEL_DIGEST}}` → `SemanticModelDigester.digest(connectionId)` 返回的 ≤ 2000 字符摘要；当 active session 未绑定 connection 时为 `<no semantic model — please bind a connection>`。
...
`com.datatalk.adapter.config.OpenCodeGatewayBeans` SHALL 在启动时为本变更新增的 **2 个** skill 各调用一次 `skillSyncer.syncSkill(name, opencodeCwd)`：`skill-creator` 与 `semantic-model-usage`。已有的 11 个 `agents-md-skills-refactor` 变更注册的 skill 与 `bezel` 注册 MUST 保留（注：`data-ingestion` 由 `script-runner` change 移除，不在本变更范围）。`classpath:/skills/` 下的目录数与 `OpenCodeGatewayBeans` 中 `syncSkill` 调用次数 MUST 一致。

#### Scenario: 2 个新 skill 全部注册
...
新增的每个 SKILL.md（本变更新增 `skill-creator`、`semantic-model-usage`；既有 11 个 + `bezel` 不在本变更修改范围）SHALL 在文件首部包含一个 YAML frontmatter 块，块内 MUST 提供 `name` 与 `description` 两个键。`name` MUST 与目录名一致。`description` MUST 同时包含中文与英文触发词（至少各 1 个），且总字符长度在 80 至 600 之间。本变更新增的 `skill-creator/SKILL.md` 额外 MUST 含 `forked_from` 字段，格式形如 `anthropics/skills@<commit-sha>`。
...
本变更不引入新的关键词归属冲突。`agents-md-skills-refactor` change 已固化的 11 个 skill 关键词归属表（`K`）保持不变。新增 `skill-creator` 与 `semantic-model-usage` 引入的新关键词（如 `Semantic Model` / `verified query` / `propose_change` / `pending`）MUST 在 `semantic-model-usage/SKILL.md` 中首次完整定义，其他 skill 如需引用 MUST 使用 `[[semantic-model-usage]]` 形式短引用。
...
任何 `classpath:/skills/<name>/SKILL.md` SHALL NOT 在文本中包含形如 `{{[A-Z_]+}}` 的占位符字面量；占位符仅允许出现在 AGENTS.md 中（因为 `AgentPromptBuilder.render()` 不处理 skill 文件）。本变更新增的 `{{SEMANTIC_MODEL_DIGEST}}` 占位符 MUST 仅出现在 AGENTS.md 中。

#### Scenario: skill 文件无任何占位符

- **WHEN** 对每个 SKILL.md（含本变更新增的 2 个）执行正则 `\{\{[A-Z_]+\}\}` 搜索
- **THEN** 匹配次数 = 0
...
- **GIVEN** AGENTS.md 已经写入精简版（含本变更新增的 1 个占位符 + 1 行 Trigger Gate + 2 行 Skill Index）
