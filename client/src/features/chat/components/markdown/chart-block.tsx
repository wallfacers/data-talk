...
function ChartError({ json, message }: { json: string; message: string }) {
  const { t } = useI18n()
  return (
...
export const ChartBlock = memo(function ChartBlock({
  ...
  const parsed = parseChartOption(json)
  if (parsed.ok) {
  ...
  const option = parsed.ok ? parsed.option : lastValidOptionRef.current
  ...
  if (!option) {
    return <ChartError json={json} message={parsed.ok ? 'invalid chart option' : parsed.error} />
  }
      ...
      <div className="flex items-center justify-between border-b border-[var(--dt-border-subtle)] bg-[var(--dt-bg-subtle)] px-3 py-1.5">
        ...
        <div className="flex items-center gap-3 text-[13px] leading-[18px]">
          <button
            ...
            title={openLabel}
            className="rounded px-1 text-[var(--dt-text-muted)] transition-colors hover:text-[var(--dt-text-strong)] disabled:cursor-not-allowed disabled:opacity-50"
          >
            {promoteState === 'loading' ? t('artifact.preparing') : openLabel}
          </button>
          <button
            type="button"
            ...
            className="rounded px-1 text-[var(--dt-text-muted)] transition-colors hover:text-[var(--dt-text-strong)]"
          >
            ⤢
          </button>
          <button
